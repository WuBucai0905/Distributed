__version__ = '0.0.1'
# bit-map:每份数据通过哈希函数映射到某一位
# bit-map冲突率高，为了降低冲突，Bloom Filter使用多个哈希函数，而不是一个

# 删除操作可以使用计数删除解决这个问题。即不再使用bit位，而存储一个数值。插入操作时不再是置1，而是加1操作。判断时不再判断0、1，而是判断是否大于0。
# 但是这种做法明显增大了占用的内存，不考虑

# 注意redis的bitmap是用  字符串  来实现的，而redis规定字符串最长为512MB（40多亿位）

# 布隆过滤器的位数组用redis的bitmap代替
...
class BloomFilter(object):
    ...
    def __init__(self, data_size, error_rate=0.001):
        ...
        self._hash_seed = [i for i in range(1, hash_num+1)] # 质数
        # 已存数据量
        self._data_count = 0
